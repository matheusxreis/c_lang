...
void change_board_state(struct Square** board, enum Pieces piece, enum Colors color, int row, int column);
void print_board(struct Square** board);
char* show_piece_name(struct Square piece);
void mount_board(struct Square** board);
void move(struct Square** board, struct Position current, struct Position target);
int move_is_valid(struct Square current_square,
...
void move(struct Square** board,
    ...
    change_board_state(board, curr.piece, curr.color, target.row, target.column);
  ...
  switch(current_square.piece) {
    case Pawn:
        if(current.row<target.row &&
            (target.column +1 == current.column || target.column -1 == current.column || target.column == current.column)) {
          return 1;
        }else {
          return 0;
        }
...
char* show_piece_name(struct Square square) {
  switch(square.piece) {
    case 1: 
      return "K";
    case 2:

...
// ======= ADDING RECURSIVELY 
void sum_w_carry(node* n1, node* n2, node** result, int carry) {
  ...
  int total = (number1 == -1 ? 0 : number1) + (number2 == -1 ? 0 : number2) + carry;
...
void sum(node* n1, node* n2, node** result) {
...
void getstr(char* str, char* temp) {
  strcpy(temp, str); 
  temp[WORD] = '\0';
}

void read_numbers(node** list1, node** list2) {

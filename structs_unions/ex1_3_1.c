...
void negative(COMPLEX* complex, COMPLEX* result){
  result->real = (-complex->real);
  result->imaginary = (-complex->imaginary); //*1 
};

void multiply(COMPLEX* comp1, COMPLEX* comp2, COMPLEX* comp3) {
  comp3->real = ((comp1->real * comp2->real) - (comp1->imaginary * comp2->imaginary));
  comp3->imaginary = ((comp1->real * comp2->imaginary) + (comp1->imaginary * comp2->real)); //*i
};

void sum(COMPLEX* comp1, COMPLEX* comp2, COMPLEX* comp3){
  comp3->real = (comp1->real+comp2->real); 
  comp3->imaginary = (comp1->imaginary+comp2->imaginary); //*i;
};

COMPLEX create(int real, int imaginary) {
  COMPLEX c;
  c.real = real;
